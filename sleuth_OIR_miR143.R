library("sleuth")
setwd("/Users/vikrants/Desktop/tak1_sleuth/")
base_dir <- "/Users/vikrants/Desktop/tak1_sleuth/"
...
s2c$condition <- relevel(s2c$condition, "OIRscrambled")
s2c
...
so <- sleuth_wt(so, 'conditionOIRmiR143')

#so <- sleuth_wt(so)

results_table <- sleuth_results(so, 'conditionOIRmiR143')

results_ordered <- results_table[order(results_table$qval),]

table(results_ordered$qval <= 0.01)
write.table(results_ordered, file = 'Sleuth.DE_OIR_scrambled_vs_OIR_miR143.txt', sep="\t",row.names=F, quote=F)
write.table( subset(results_ordered, qval <= 0.01), file='sleuth.DE_transcripts.qval_0.01_OIR_scrambled_vs_OIR_mir143_new.txt', sep="\t",row.names=F, quote=F)
