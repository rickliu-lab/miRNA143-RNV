...
so <- sleuth_wt(so, 'conditionmiR143')

#so <- sleuth_wt(so)

results_table <- sleuth_results(so, 'conditionmiR143')

results_ordered <- results_table[order(results_table$qval),]

table(results_ordered$qval <= 0.01)
write.table(results_ordered, file = 'Sleuth.DE_Control_vs_miR143.txt', sep="\t",row.names=F, quote=F)
write.table( subset(results_ordered, qval <= 0.01), file='sleuth.DE_transcripts.qval_0.01_control_vs_143_new.txt', sep="\t",row.names=F, quote=F)
