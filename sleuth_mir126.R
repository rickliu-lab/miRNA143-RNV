...
sample_id <- dir(file.path(base_dir,"results_2/"))
sample_id


kal_dirs <- sapply(sample_id, function(id) file.path(base_dir, "results_2", id, "kallisto"))
kal_dirs



s2c <- read.table(file.path(base_dir, "hiseq_info_2.txt"), header = TRUE, stringsAsFactors=FALSE)
...
so <- sleuth_wt(so, 'conditionmiR126')

#so <- sleuth_wt(so)

results_table <- sleuth_results(so, 'conditionmiR126')

results_ordered <- results_table[order(results_table$qval),]

table(results_ordered$qval <= 0.01)
write.table(results_ordered, file = 'Sleuth.DE_Control_vs_miR126.txt', sep="\t",row.names=F, quote=F)
write.table( subset(results_ordered, qval <= 0.01), file='sleuth.DE_transcripts.qval_0.01_control_vs_126_new.txt', sep="\t",row.names=F, quote=F)
